...
class _FileExplorer extends State<FileExplorer> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
              ...
              builder: (context, heldPaths, child) => ValueListenableBuilder(
                ...
                builder: (context, selectedPaths, child) => Row(
                  children: selectedPaths.isNotEmpty
                      ...
                      : [
                          // Botón para crear archivo con commit (NUEVO)
                          IconButton(
                            onPressed: () => _showCreateFileWithCommitDialog(),
                            style: ButtonStyle(
                              tapTargetSize: MaterialTapTargetSize.shrinkWrap,
                              padding: WidgetStatePropertyAll(EdgeInsets.all(spaceXXS)),
                            ),
                            icon: FaIcon(FontAwesomeIcons.fileCirclePlus, color: primaryLight, size: textLG),
                            tooltip: 'Create file with commit',
                          ),
                          SizedBox(width: spaceXXS),
                          // Botón para crear archivo simple (existente)
                          IconButton(
                            onPressed: () async {
                            ...
                            icon: FaIcon(FontAwesomeIcons.file, color: primaryLight, size: textLG),
                            tooltip: 'Create empty file',
                          ),
                          SizedBox(width: spaceXXS),
                          // Botón para crear carpeta (existente)
                          IconButton(
                            onPressed: () async {
        ...
        // Botón flotante para crear archivo con commit (NUEVO)
        floatingActionButton: FloatingActionButton(
          onPressed: () => _showCreateFileWithCommitDialog(),
          backgroundColor: primaryInfo,
  ...
  // Método para mostrar diálogo de crear archivo con commit
  Future<void> _showCreateFileWithCommitDialog() async {
    await CreateFileDialog.showCreateFileWithCommitDialog(
      context,
      widget.path, // repositoryPath
      controller.getCurrentPath, // currentDirectory
    );
    
    // Refrescar la vista después de crear el archivo
    final destinationPath = controller.getCurrentPath;
